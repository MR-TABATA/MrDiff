...
**何でも差分。テキストも画像もバイナリもサイトも。一発で「違うか」「どこが」がわかる。**
...
| 780 MB のログ、1,000 万行 | 2 行、キャッシュ済み | **0.70 s** | 0.68 s |
| 780 MB のログ、1,000 万行 | 無し、キャッシュ済み | 0.67 s | 0.65 s |

*（M4 Max、release ビルドで計測。バイナリの行は 2026-09-11、ログの行は 2026-09-13。
1 行目は 1 GB のファイル 2 つをコールドで読んだもの。ほかはページキャッシュにある ──
バイナリの組は仕事が比較ではなく I/O にあることを、ログの組は 1,000 万行のテキスト diff が
バイトさえメモリにあれば 1 秒かからないことを示す。コールドなら読む時間が乗る。この機械で
1 GB あたり約 4 秒。）*

ログは `mmap` で読むので、メモリより大きいファイルでも動く。比べるためだけにバッファへ
写すことはなく、どこにも送らない ── 機密のログも、校正刷りも、スクリーンショットも、
URL を渡さない限り手元から出ない。

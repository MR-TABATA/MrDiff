...
# --version が言う版とタグが違う配布物は出さない。
DECLARED=$(sed -n 's/^let mrdiffVersion = "\(.*\)"$/\1/p' Sources/mrdiff/Version.swift)
if [ "$DECLARED" != "$VERSION" ]; then
  echo "Sources/mrdiff/Version.swift は ${DECLARED}、配る版は ${VERSION}。先に Version.swift を直すこと" >&2
  exit 1
fi
